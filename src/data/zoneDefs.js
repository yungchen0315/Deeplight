/* ============================================================================
 * zoneDefs.js — 十四段海域主幹（企劃書第 4a 節 + v1.1~Phase9 擴充）。depth 會被夾在
 ...
 * gateCost 曲線：0~3 段是上一輪依實測回饋調整過的（見當時的模擬結果），4~12 段
 * 用同一套模擬工具（含研究樹/模組升級/壓載）校正，把「時間到下一海域」的成長率
 * 抹平成穩定的 1.3~2.5 倍區間，取代原本忽快忽慢的曲線。id 12 的 gateCost 這次隨著
 * 第 13 海域（本源塹）正式開放重新校正過（模擬結果：z11→z12 約 129.9h，z12→z13 約
 * 186.2h，比例 1.43x，落在同一區間內）。id 13 的 gateCost 目前無法被觸發（下一個
 * 海域是 comingSoon 的預告牌，descentSystem.passGate 會擋下），先延續同一套倍率
 * 保留一個佔位數值，等真的開放第 15 海域時再一併重新校正。
 * ==========================================================================*/
(function () {
  const ZONE_DEFS = [
    ...
    { id: 13, name: '本源塹', minDepth: 840000, anchorDepth: 1300000, mult: 32000000, bg: 'INK', filterHue: 260, gateCost: 9e40 },
    { id: 14, name: '深海溝底．即將開放', minDepth: 1300000, anchorDepth: 1300000, mult: 32000000, bg: 'INK', filterHue: 260, gateCost: null, comingSoon: true }
  ];
