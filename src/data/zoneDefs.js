/* ============================================================================
 * zoneDefs.js — 十五段海域主幹（企劃書第 4a 節 + v1.1~Phase10 擴充）。depth 會被夾在
 ...
 * 抹平成穩定的 1.3~2.5 倍區間，取代原本忽快忽慢的曲線。id 13 的 gateCost 這次隨著
 * 第 14 海域（彼岸）正式開放重新校正過（模擬結果：z12→z13 約 186.3h，z13→z14 約
 * 261.1h，比例 1.40x，落在同一區間內）。id 14 的 gateCost 目前無法被觸發（下一個
 * 海域是 comingSoon 的預告牌，descentSystem.passGate 會擋下），先延續同一套倍率
 * 保留一個佔位數值，等真的開放第 16 海域時再一併重新校正。
 * ==========================================================================*/
(function () {
  const ZONE_DEFS = [
    ...
    { id: 14, name: '彼岸', minDepth: 1300000, anchorDepth: 1950000, mult: 128000000, bg: 'INK', filterHue: 180, gateCost: 4e43 },
    { id: 15, name: '深海溝底．即將開放', minDepth: 1950000, anchorDepth: 1950000, mult: 128000000, bg: 'INK', filterHue: 180, gateCost: null, comingSoon: true }
  ];
