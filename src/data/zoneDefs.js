/* ============================================================================
 * zoneDefs.js — 十六段海域主幹（企劃書第 4a 節 + v1.1~Phase11 擴充）。depth 會被夾在
 ...
 * 抹平成穩定的 1.3~2.5 倍區間，取代原本忽快忽慢的曲線。id 14 的 gateCost 這次隨著
 * 第 15 海域（歸墟）正式開放重新校正過——原本 4e43 只是預告牌占位值，模擬顯示會造成
 * z14→z15 高達 3.38x 的斷崖，重新求解為 1.5e43（模擬結果：z13→z14 約 261h，z14→z15
 * 約 408h，比例 1.53x，落在同一區間內）。id 15 的 gateCost 目前無法被觸發（下一個
 * 海域是 comingSoon 的預告牌，descentSystem.passGate 會擋下），先延續同一套倍率
 * 保留一個佔位數值，等真的開放第 17 海域時再一併重新校正。
 * ==========================================================================*/
(function () {
  const ZONE_DEFS = [
    ...
    { id: 15, name: '歸墟', minDepth: 1950000, anchorDepth: 2900000, mult: 512000000, bg: 'INK', filterHue: 210, gateCost: 1.1e46 },
    { id: 16, name: '未知深淵．即將開放', minDepth: 2900000, anchorDepth: 2900000, mult: 512000000, bg: 'INK', filterHue: 210, gateCost: null, comingSoon: true }
  ];
